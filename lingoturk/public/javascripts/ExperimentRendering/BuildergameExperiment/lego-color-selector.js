...
var params = {
    color: LegoColors.Green, // Default color
    colors: Object.keys(LegoColors), // Available colors
    selectedColor: 'Green'
};
...
export function init() {
    ...
    renderer.setSize(window.innerWidth - 300, window.innerHeight); // Subtract left panel width
    document.querySelector('.canvas-container').appendChild(renderer.domElement);

    // Set up dat.gui
    var gui = new dat.GUI();
    gui.add(params, 'selectedColor', params.colors)
       .onChange(function(value) {
           // Update hover guide color
           hoverLegoMaterial.color.setHex(LegoColors[value]);
           params.color = LegoColors[value];
           render();
       });
...
function load_existing_structure(){

     const lego_params = [
      {x: 25, y: 25, z: 25, color: LegoColors.Red },
      {x: 25, y: 75, z: 25, color: LegoColors.Blue1},
      {x: 25, y: 125, z: 25, color: LegoColors.Green}
     ]
     // Create a lego brick with the selected color
     for (var i = 0; i < lego_params.length; i++) {
         legoMaterial = new THREE.MeshLambertMaterial({ color:lego_params[i].color });
         const legoBrick = new THREE.Mesh(legoGeometry, legoMaterial);

         legoBrick.position.set(lego_params[i].x, lego_params[i].y, lego_params[i].z)
         scene.add(legoBrick);
...
function onPointerDown(event) {
    ...
    if (intersections.length > 0) {
        ...
        if (isShiftKeyDown) {
            ...
            // Create a lego brick with the selected color
            legoMaterial = new THREE.MeshLambertMaterial({ color: params.color });
            const legoBrick = new THREE.Mesh(legoGeometry, legoMaterial);
